...
| 前端 | Bootstrap 5 + Bootstrap Icons + Quill.js |
| 安全 | Rate Limiting + SecurityHeaders + AntiForgery |
...
| 角色 | Area | 說明 | 帳號 |
|------|------|------|------|
| 管理員 | `/Admin` | 系統管理、用戶管理、所有課程 | admin@koreanlearn.com |
| 教師 | `/Teacher` | 管理自己的課程、章節、單元 | teacher@koreanlearn.com |
| 學生 | `/Learn` | 學習、進度追蹤、測驗、字卡 | student@koreanlearn.com |
...
- [x] 25 個 Entity + ISoftDeletable + BaseEntity + Fluent API
- [x] ApplicationDbContext（軟刪除攔截、Global Query Filter、自動時戳）
- [x] ASP.NET Core Identity（三角色：Admin / Teacher / Student）
- [x] Repository + UnitOfWork + IDbContextFactory
- [x] Areas（Admin / Teacher / Learn / Identity）
...
- **N+1 查詢優化**：批次載入取代逐筆查詢
- **Serilog 結構化日誌**：Console + File 輪替（30 天）
